...
class AskFacultyName(Action):
    def name(self) -> Text:
        return "ask_faculty_id"

    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text,Any]]:
        str((tracker.latest_message)['text'])
        dispatcher.utter_message(template="utter_ask_faculty_id")
        return []

class DisplayFacultyDetails(Action):
    def name(self) -> Text:
        return "display_faculty_details"
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text,Any]]:
        ...
        fac_id = str((tracker.latest_message)['text'])

        conn = sqlite3.connect('University.db')
        query = "select * from Faculty_details where facid = '{0}'".format(fac_id)
        ...
        if df.shape[0] == 1:
            ...
            content = "Sorry the enetered ID is not valid"
...
class DisplayUpcomingClubEvents(Action):

    def name(self) -> Text:
        return "display_upcoming_club_events"
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        print("Today's date:", today)
        this_month = today.strftime("%m")
        df2 = pd.read_excel('Club_events.xlsx')
        df2['Date'] = pd.to_datetime(df2['Date'])
        current_month_df = df2[df2['Date'].dt.month == int(this_month)]
        content = 'Total of ' + str(current_month_df.shape[0]) + ' this month\n\n'
        for i in range(current_month_df.shape[0]):
            content = content + str(current_month_df['Date'].values[i])[:10] + '  -  ' + str(
                current_month_df['Club Name'].values[i]) + " - "+ str(
                current_month_df['Nature of Events'].values[i]) +  '\n'
